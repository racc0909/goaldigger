...
st.sidebar.header(f'Personal Info of {profile.user_nickname}')
st.sidebar.number_input('Your current age is', value = current_age)
...
selected_country = st.sidebar.selectbox('Select your country:', list(country_data.keys()), index=list(country_data.keys()).index(profile.user_country) if profile.user_country else 0)
